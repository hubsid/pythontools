...
@click.command()
@click.option('--duration', type=int, default=24, required=True, help='duration to extend the deployment, in hours. If a deployment\n'
...
@click.option('--lastn', default=0, help='if the option --deployment-id is not given, then this option is used to select the nth active deployment\n'
                                           'in case of multiple active deployments')
def main(duration, deployment_id, lastn):
    """
    \b
    ...
    """
    success, result = rdmedit.main(duration, deployment_id, lastn)
